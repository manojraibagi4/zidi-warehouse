 ...
 * Supports both English and German column headers.
 *
...
function handleExcelImport(string $filePath, string $tableName): array
{
    $tableMappings = [
        'items' => [
            ...
            // German headers (multiple variations supported)
            'Produktname'           => ['db_column' => 'productname',     'type' => 's'],
            'Bezeichnung'           => ['db_column' => 'productname',     'type' => 's'],
            'Kategorie'             => ['db_column' => 'category',        'type' => 's'],
            'Artikelnummer'         => ['db_column' => 'article_no',      'type' => 's'],
            'Artikel-Nr.'           => ['db_column' => 'article_no',      'type' => 's'],
            'Artikel-Nr'            => ['db_column' => 'article_no',      'type' => 's'],
            'Identifier'            => ['db_column' => 'article_no',      'type' => 's'],
            'Hersteller'            => ['db_column' => 'manufacturer',    'type' => 's'],
            'Firma'                 => ['db_column' => 'manufacturer',    'type' => 's'],
            'Beschreibung'          => ['db_column' => 'description',     'type' => 's'],
            'Größe'                 => ['db_column' => 'size',            'type' => 's'],
            'Farbe'                 => ['db_column' => 'color',           'type' => 's'],
            'Farbnummer'            => ['db_column' => 'color_number',    'type' => 's'],
            'Farb-Nr.'              => ['db_column' => 'color_number',    'type' => 's'],
            'Farb-Nr'               => ['db_column' => 'color_number',    'type' => 's'],
            'Einzelpreis'           => ['db_column' => 'unit_price',      'type' => 'd'],
            'EK / Stk. Netto'       => ['db_column' => 'unit_price',      'type' => 'd'],
            'EK/Stk. Netto'         => ['db_column' => 'unit_price',      'type' => 'd'],
            'Gesamtpreis'           => ['db_column' => 'total_price',     'type' => 'd'],
            'Summe'                 => ['db_column' => 'total_price',     'type' => 'd'],
            'Lieferant'             => ['db_column' => 'supplier',        'type' => 's'],
            'Menge'                 => ['db_column' => 'quantity',        'type' => 'i'],
            'Anzahl'                => ['db_column' => 'quantity',        'type' => 'i'],
            'Veredelt'              => ['db_column' => 'grafted',         'type' => 'b'],
            'Verein'                => ['db_column' => 'club',            'type' => 's'],
            'Ablaufjahr'            => ['db_column' => 'expiration_year', 'type' => 'y'],
            'Ablaufdatum'           => ['db_column' => 'expiry_date',     'type' => 'date'],
            'Bild'                  => ['db_column' => 'img',             'type' => 'blob'],
            'Letzte Änderung'       => ['db_column' => 'last_change',     'type' => 'dt'],
            'Zuletzt bearbeitet von' => ['db_column' => 'last_edited_by', 'type' => 's'],
            'MIME-Typ'              => ['db_column' => 'mime_type',       'type' => 's'],
        ],
    ];
        ...
        for ($row = 2; $row <= $highestRow; $row++) {
            ...
            for ($col = 1; $col <= $highestColIndex; $col++) {
                ...
                if ($dbColumn === 'img') {
                    ...
                    // Support both English (True/False) and German (Ja/Nein) boolean values
                    $lowerValue = strtolower(trim($cellValue));
                    $processedValue = in_array($lowerValue, ['true', 'ja', '1', 'yes']) ? 1 : 0;
                } elseif ($type === 'y') {
                ...
                if ($header === $primaryKeyHeader || $dbColumn === $primaryKeyDbColumn) {
            ...
            $bindValues = [];
            $bindTypes = '';
            $processedDbColumns = []; // Track which db columns we've already added
            ...
            foreach ($excelHeaders as $colIndex => $header) {
                ...
                // Skip if this database column was already added (handles duplicate mappings)
                if (in_array($dbCol, $processedDbColumns)) continue;

                $dbColumns[] = $dbCol;
                $processedDbColumns[] = $dbCol;
                $val = $rowData[$header];
                $bindValues[] = $val;
            ...
            $imageHeaderFound = false;
            foreach ($rowData as $hdr => $val) {
                if (isset($mapping[$hdr]) && $mapping[$hdr]['db_column'] === 'img' && $val !== null) {
                    $imageHeaderFound = true;
                    break;
                }
            }
            
            if ($imageHeaderFound && !in_array('mime_type', $dbColumns)) {
                $dbColumns[] = 'mime_type';
